 ...
 */
public class _148_MinDistanceBetweenTwoNumbers {

	/*
	 Method 1 (Simple)
     Use two loops: The outer loop picks all the elements of arr[] one by one. The inner loop picks all the elements after the element 
     picked by outer loop. If the elements picked by outer and inner loops have same values as x or y then if needed update the minimum 
	...
	/*
     ...
     2) Now traverse arr[] after the index prev. If the element at current index i matches with either x or y then check if it is 
        different from arr[prev]. If it is different then update the minimum distance if needed. If it is same then update prev i.e., make prev = i.
	 */
	...
	public static void main(String[] args) { 
		_148_MinDistanceBetweenTwoNumbers min = new _148_MinDistanceBetweenTwoNumbers(); 
