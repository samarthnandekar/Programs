...
public class _76_ChunkMerge {
    ...
    public int[] mergeChunksOfDifferentSize1(List<List<Integer>> chunks)
    {
    	int totalEl=0;
    	 for(List<Integer> chunk : chunks){
    		 totalEl=totalEl+chunk.size();
    	 }
    	 int [] arr=new int[totalEl];
    	 int i=0;
    	 for(List<Integer> chunk : chunks)
    	 {
    		 for(int currNum:chunk)
    		 {
    			 arr[i]=currNum;
    			 i++;
    		 }
    ...
    private void mergeSort1(int[] result,int start,int end){
        if(start >= end){
            return;
        }
        int mid = (start + end)/2;
        mergeSort1(result,start,mid);
        mergeSort1(result,mid+1,end);
    ...
    public static void main(String args[]){
        ...
        _76_ChunkMerge cm = new _76_ChunkMerge();
      ...
      //  result = cm.mergeUsingHeap(chunks);
        int[] result1 = cm.mergeChunksOfDifferentSize1(chunks);
        System.out.println();
