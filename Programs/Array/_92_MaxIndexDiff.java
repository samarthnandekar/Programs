...
/*
 https://github.com/samarthnandekar/Programs/blob/master/Programs/Array/MaxIndexDiff.java
...
public class _92_MaxIndexDiff {

	/* Method 1 (Simple but Inefficient) :- Run two loops. In the outer loop, pick elements one by one from left. In the inner loop,
	  compare the picked element with the elements starting from right side. Stop the inner loop when you see an element greater than 
	  the picked element and keep updating the maximum j-i so far. Time Complexity : O(n^2) */	
	...
	/* Method 2 (Efficient) :- To solve this problem, we need to get two optimum indexes of arr[]: left index i and right index j. For an element arr[i], 
    ...
    int maxIndexDiffSec(int arr[], int n)  
    { 
        int maxDiff; 
        int i, j; 
    ...
    { 
    	_92_MaxIndexDiff max = new _92_MaxIndexDiff(); 
        int arr[] = {9, 2, 3, 4, 5, 6, 7, 8, 18, 0};         
        int n = arr.length; 
        int maxDiff = max.maxIndexDiffSec(arr, n); 
